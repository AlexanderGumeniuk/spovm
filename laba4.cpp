...
void AddThread()
{
	...
	(*arg).num = threads.size();              // Íîìåð äîáàâëÿåìîãî ïîòîêà
	(*arg).quitFlag = quitFlags.back();		  // Óêàçàòåëü íà ôëàã çàêðûòèÿ äëÿ äàííîãî ïîòîêà
...
unsigned int __stdcall  printString(void* arg)
{
	...
	while (1)
	{
		...
		for (int i = 0; i < strlen(strings[threadNumber]); i++)
		{
			...
			Sleep(100);
			if (i == strlen(strings[threadNumber]) - 1) 
			{
				printf("\n");
			}
		}
